...
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> &image, double td)         // DUY ADD
{
    RCLCPP_DEBUG(node->get_logger(), "%s input feature: %d", AT, (int)image.size());
    RCLCPP_DEBUG(node->get_logger(), "%s num of feature: %d", AT, getFeatureCount());
    ...
    for (auto &id_pts : image)
    {
        ...
        if (it == feature.end())
        {
            ...
            feature.push_back(FeaturePerId(feature_id, frame_count, f_per_fra.depth)); // DUY ADD
    ...
    {
        RCLCPP_DEBUG(node->get_logger(), "%s parallax_sum: %lf, parallax_num: %d", AT, parallax_sum, parallax_num);
        RCLCPP_DEBUG(node->get_logger(), "%s current parallax: %lf", AT, parallax_sum / parallax_num * FOCAL_LENGTH);
...
{
    RCLCPP_DEBUG(node->get_logger(), "%s debug show", AT);
    for (auto &it : feature)
    {
        ...
        RCLCPP_DEBUG(node->get_logger(), "%s %d,%d,%d", AT, it.feature_id, it.used_num, it.start_frame);
        int sum = 0;
        for (auto &j : it.feature_per_frame)
        {
            RCLCPP_DEBUG(node->get_logger(), "%s %d,", AT, int(j.is_used));
            sum += j.is_used;
...
void FeatureManager::setDepth(const VectorXd &x)
{
    ...
    for (auto &it_per_id : feature)
    {
        ...
        //RCLCPP_INFO(node->get_logger(), "%s feature id %d , start_frame %d, depth %f ", AT, it_per_id->feature_id, it_per_id-> start_frame, it_per_id->estimated_depth);
...
void FeatureManager::clearDepth(const VectorXd &x)
{
    ...
    for (auto &it_per_id : feature)
    {
        ...
        it_per_id.lidar_depth_flag = false; // DUY ADD 
    }
}
...
void FeatureManager::triangulate(Vector3d Ps[], Vector3d tic[], Matrix3d ric[])
{
    for (auto &it_per_id : feature)
    {
        ...
        // depth is available, skip triangulation (trust the first estimate) // DUY ADD
        if (it_per_id.estimated_depth > 0)
            continue;
...
void FeatureManager::triangulate_depth(Vector3d Ps[], Vector3d tic[], Matrix3d ric[])
{
    for (auto &it_per_id : feature)
    {
        it_per_id.used_num = it_per_id.feature_per_frame.size();
...
void FeatureManager::removeBackShiftDepth(Eigen::Matrix3d marg_R, Eigen::Vector3d marg_P, Eigen::Matrix3d new_R, Eigen::Vector3d new_P)
{
         ...
         it != feature.end(); it = it_next)
    {
        ...
        else
        {
            ...
            {
                Eigen::Vector3d pts_i = uv_i * depth;    // feature in cartisian space in old local camera frame // DUY ADD
