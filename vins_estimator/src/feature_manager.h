...
{
  public:
    FeaturePerFrame(const Eigen::Matrix<double, 8, 1> &_point, double td)         // DUY ADD
    {
        point.x() = _point(0);
        point.y() = _point(1);
        point.z() = _point(2);
...
class FeatureManager
{
  public:
    ...
    bool addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> &image, double td);         // DUY ADD
    void debugShow();
    ...
    void triangulate_depth(Vector3d Ps[], Vector3d tic[], Matrix3d ric[]);
