...
// global variable saving the lidar odometry // DUY ADD
deque<nav_msgs::msg::Odometry> odomQueue;
odometryRegister *odomRegister;
std::mutex m_odom; // DUY ADD
...
{
    double t = Utility::toSec(imu_msg->header.stamp);
    if (init_imu)
    {
        latest_time = t;
        init_imu = 0;
...
std::vector<std::pair<std::vector<sensor_msgs::msg::Imu::SharedPtr>, sensor_msgs::msg::PointCloud::ConstSharedPtr>>
getMeasurements()
{
    std::vector<std::pair<std::vector<sensor_msgs::msg::Imu::SharedPtr>, sensor_msgs::msg::PointCloud::ConstSharedPtr>> measurements;
    ...
    while (true)
    {
        ...
        if (!(Utility::toSec(imu_buf.back()->header.stamp) > Utility::toSec(feature_buf.front()->header.stamp) + estimator.td))
        {
            // RCLCPP_WARN(node->get_logger(), "%s wait for imu, only should happen at the beginning", AT);
            sum_of_wait++;
            return measurements;
        }
        ...
        while (Utility::toSec(imu_buf.front()->header.stamp) < Utility::toSec(img_msg->header.stamp) + estimator.td)
        {
        ...
        IMUs.emplace_back(imu_buf.front());
        if (IMUs.empty())
            RCLCPP_WARN(node->get_logger(), "%s no imu between two image", AT);
...
void imu_callback(sensor_msgs::msg::Imu::SharedPtr imu_msg)
{
    ...
    last_imu_t = Utility::toSec(imu_msg->header.stamp);
...
void feature_callback(sensor_msgs::msg::PointCloud::ConstSharedPtr feature_msg)
{
    if (!init_feature)
    {
...
void restart_callback(std_msgs::msg::Bool::SharedPtr restart_msg)
{
    ...
    {
        RCLCPP_WARN(node->get_logger(), "%s restart the estimator!", AT);
        m_buf.lock();
...
void relocalization_callback(sensor_msgs::msg::PointCloud::ConstSharedPtr points_msg)
{
...
void process()
{
    while (true)
    {
        ...
        for (auto &&measurement : measurements)
        {
            ...
            {
                double t = Utility::toSec(imu_msg->header.stamp);
                double img_t = Utility::toSec(img_msg->header.stamp) + estimator.td;
                if (t <= img_t)
                { 
                    if (current_time < 0)
                        current_time = t;
            ...
            if (relo_msg != NULL)
            {
                ...
                double frame_stamp = Utility::toSec(relo_msg->header.stamp);
            ...
            // 2. VINS Optimization
            TicToc t_s;
            map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> image;       // DUY ADD
            for (unsigned int i = 0; i < img_msg->points.size(); i++)
            {
                ...
                double depth = img_msg->channels[5].values[i]; // DUY ADD

                assert(z == 1);
                Eigen::Matrix<double, 8, 1> xyz_uv_velocity_depth;           // DUY ADD
            ...
            m_odom.lock();
            initialization_info = odomRegister->getOdometry(odomQueue, Utility::toSec(img_msg->header.stamp) + estimator.td);
            m_odom.unlock();
            ...
            // RCLCPP_ERROR(node->get_logger(), "%s end: %f, at %f", AT, Utility::toSec(img_msg->header.stamp), rclcpp::Clock().now().nanoseconds() / 1e9);
        }
        m_estimator.unlock();
        m_buf.lock();
        m_state.lock();
...
int main(int argc, char **argv)
{
    ...
    RCLCPP_DEBUG(node->get_logger(), "%s EIGEN_DONT_PARALLELIZE", AT);
#endif
    RCLCPP_DEBUG(node->get_logger(), "%s waiting for image and imu...", AT);
    ...
    // auto sub_odom = node->create_subscription<nav_msgs::msg::Odometry>("/lio_sam/mapping/odometry",rclcpp::SensorDataQoS(),odom_callback); // DUY ADD
